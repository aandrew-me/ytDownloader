...
function download(type) {
	...
	if (type === "video") {
		const args = [
			...
			subs,
			subLangs,
			videoType == "mp4" && (url.includes("youtube.com/") || url.includes("youtu.be/")) ? "--embed-thumbnail" : "",
		...
		if (
			...
			const args = [
				...
				subs,
				subLangs,
				format === "mp3" || format === "m4a" && (url.includes("youtube.com/") || url.includes("youtu.be/")) ? "--embed-thumbnail" : "",
				proxy ? "--no-check-certificate" : "",
				proxy ? "--proxy" : "",
				proxy,
				"--compat-options",
